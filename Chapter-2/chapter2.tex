...
\begin{algorithmic}[1]
\STATE \textbf{Input:} Data window $\mathbf{X} = \{\mathbf{x}_t\}_{t=1}^{T} \subset \mathbb{R}^d$, 
predicted activity $a$, coreset size $m$, temperature $\tau$, available energy $E_{\text{available}}$
\STATE \textbf{Output:} Coreset $S = \{(\mathbf{s}_i, w_i)\}_{i=1}^{m}$

\STATE $S \leftarrow \emptyset$

\STATE $\boldsymbol{\mu}_a \leftarrow \text{ActivityPrototype}(a)$ 
\COMMENT{Activity-specific prototype}

\FOR{$t = 1$ to $T$}
    \STATE $d_t \leftarrow \|\mathbf{x}_t - \boldsymbol{\mu}_a\|_2$
\ENDFOR

\IF{$E_{\text{available}} \geq E_{\text{cluster}}$}
    \STATE $(\{\mathbf{c}_i\}_{i=1}^{m}, \{\mathcal{C}_i\}_{i=1}^{m}) 
    \leftarrow \text{KMeans}(\mathbf{X}, m)$

    \FOR{$i = 1$ to $m$}
        \STATE $\mathbf{s}_i \leftarrow \mathbf{c}_i$
        \STATE $w_i \leftarrow |\mathcal{C}_i|$
        \STATE $S \leftarrow S \cup \{(\mathbf{s}_i, w_i)\}$
    \ENDFOR
\ELSE
    \STATE $p_t \leftarrow 
    \frac{\exp(d_t / \tau)}{\sum_{j=1}^{T} \exp(d_j / \tau)} \quad \forall t$

    \STATE $\mathcal{I} \leftarrow 
    \text{SampleWithoutReplacement}(m, \{p_t\}_{t=1}^{T})$

    \FOR{$i = 1$ to $m$}
        \STATE $\mathbf{s}_i \leftarrow \mathbf{x}_{\mathcal{I}_i}$
        \STATE $w_i \leftarrow \frac{1}{m\,p_{\mathcal{I}_i}}$
        \STATE $S \leftarrow S \cup \{(\mathbf{s}_i, w_i)\}$
    \ENDFOR
\ENDIF
