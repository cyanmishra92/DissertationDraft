...
\begin{figure}[!htb]
    ...
    \caption{An example wildlife classification deployment---analog image sensor data is stored in digital memory, then processed in crossbar arrays in the mixed domain. The MCU, in the digital domain, performs activation functions. Every DNN layer undergoes expensive domain transfers. Our approach eliminates these costly transfers by keeping computations in the analog domain across multiple layers.}
    \label{fig:lreye-intro-diagram}
\end{figure}
...
\begin{figure}[!htb]
    ...
    \caption{An understanding of the current state of the art: (top) Different available architectures for deploying edge intelligence including PIM-based and microcontroller-based approaches; (bottom) Contribution of the different moving parts of the edge ecosystem towards inference accuracy, area, power, and efficiency. The analysis reveals significant opportunities for improvement in analog processing and hardware-aware training.}
    \label{fig:lreye-combined-analysis}
\end{figure}
...
\begin{subfigure}[t]{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{Chapter-5/LREyE-MICRO/figs/acc.pdf}
    \caption{Accuracy}
    \label{fig:lreye-training-acc}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{Chapter-5/LREyE-MICRO/figs/loss.pdf}
    \caption{Loss}
    \label{fig:lreye-training-loss}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{Chapter-5/LREyE-MICRO/figs/reg_gamma.pdf}
    \caption{Gamma Regularization}
    \label{fig:lreye-training-gamma}
\end{subfigure}
\caption{Training dynamics for hardware-aware neural network training: (a) Accuracy convergence showing stable learning despite hardware noise injection; (b) Loss curve demonstrating smooth optimization; (c) $\gamma$-regularization trends showing learned IR drop compensation factors converging to optimal values.}
\label{fig:lreye-training-curves}
\end{figure}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.8\textwidth]{Chapter-5/LREyE-MICRO/NewResultFigs/accuracy_vs_noise_trend.pdf}
\caption{Accuracy degradation with increasing hardware variation (noise). Hardware-aware training maintains high accuracy even under significant noise conditions, while standard training without hardware modeling catastrophically fails as noise increases.}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=\linewidth]{Chapter-5/LREyE-MICRO/NewResultFigs/FullDesign.pdf}
    \caption{Overall design of the proposed system: (Left) High-level hardware architecture showing the tile-based crossbar organization with Schottky diode activation circuits, ADC/DAC peripherals, and MeF-RAM buffers; (Right) The intricate hardware-software co-design and training flow that incorporates IR drop modeling, thermal noise injection, and device variability into the learning process.}
    \label{fig:lreye-full-design}
\end{figure}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=\linewidth]{Chapter-5/LREyE-MICRO/figs/samples.pdf}
    \caption{Sample images from the NTLNP (Northeast Tiger and Leopard National Park) dataset showing infrared camera trap captures of various wildlife species in different environmental conditions. The dataset includes rare and endangered species such as Amur tigers and leopards, making accurate classification critical for conservation efforts.}
    \label{fig:lreye-ntlnp-samples}
\end{figure}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.9\textwidth]{Chapter-5/LREyE-MICRO/NewResultFigs/Accuracy_vs_noise_EH.pdf}
\caption{Classification accuracy at different signal-to-noise ratios under energy harvesting conditions. Hardware-aware training maintains high accuracy across wide operating ranges, demonstrating robustness to environmental variations encountered in real-world deployments.}
\label{fig:lreye-snr-robustness}
\end{figure}
...
\begin{subfigure}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{Chapter-5/LREyE-MICRO/NewResultFigs/model_accuracy_final.pdf}
    \caption{Accuracy comparison across models}
    \label{fig:lreye-accuracy-comparison}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{Chapter-5/LREyE-MICRO/NewResultFigs/power_efficiency_comparison.pdf}
    \caption{Power efficiency comparison (GOps/W)}
    \label{fig:lreye-power-comparison}
\end{subfigure}
\caption{Quantitative comparison with state-of-the-art approaches: (a) Classification accuracy showing our hardware-aware training recovers most of the accuracy loss from analog non-idealities; (b) Power efficiency demonstrating significant improvements over digital and single-layer analog approaches.}
\label{fig:lreye-comparison}
\end{figure}

\begin{figure}[!htb]
\centering
\includegraphics[width=0.9\textwidth]{Chapter-5/LREyE-MICRO/NewResultFigs/network_latency_comparison.pdf}
\caption{Latency breakdown and savings for four representative networks. Our two-layer analog pipeline significantly reduces ADC/DAC conversion overhead, resulting in 20-40\% latency reduction compared to single-layer analog approaches.}
\label{fig:lreye-latency-comparison}
\end{figure}
