...
For image data, we consider the Fashion-MNIST~\cite{fmnist} and CIFAR10~\cite{cifar10} datasets; for time series sensor data, we focus on popular human activity recognition (HAR) datasets~\cite{HARHompel, HARHaChoi}, MHEALTH~\cite{mHealth, mHealthDroid} and PAMAP2~\cite{pamap2, PAMAP21, PAMAP22}; and for audio, we use the AudioMNIST~\cite{audioMNIST} dataset.
